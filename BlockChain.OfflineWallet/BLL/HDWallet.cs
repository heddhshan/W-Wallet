...
";
        ...
        public static System.Data.DataTable GetHDWallets()
        {
            string sql = @"
SELECT   MneHash, MneId, MneAlias,
                    (SELECT   COUNT(*) AS AN
                     FROM      HD_Address
...
";
        ...
        public static Model.HD_Mnemonic NewHDWallet(string alias, string password, string tip, string mne = "", bool isBackUp = false )
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    //model.HasPrivatekey = false;            //第 0 个地址，主要用于验证，不用于交易！ 实际上是有私钥的，但标记为无私钥！  离线钱包无所谓的！

                    model.ValidateEmptyAndLen();
                    DAL.HD_Mnemonic.Insert(Share. ShareParam.DbConStr, model);

                    SaveNewHdAddress(model, mne, Share. LanguageHelper.GetTranslationText( "第0个地址（主地址）"), 0);    //使用第 0 个，是为了和其他钱包兼容。
        ...
        public static void SaveNewHdAddress(Model.HD_Mnemonic model, string mne, string alias, int accountIndex)
        {
        ...
        public static Model.HD_Mnemonic GetHdModel(string hdAddress)
        {
            string conStr = Share.ShareParam.DbConStr;
            string sql = @"
SELECT H.*
FROM   HD_Address AS A INNER JOIN
          HD_Mnemonic AS H ON A.MneId = H.MneId
...
";
            ...
            cm.CommandType = System.Data.CommandType.Text;
            cm.CommandText = sql;
            cm.Parameters.Add("@Address", SqlDbType.NVarChar).Value = hdAddress;
